      ...
      describe.each([
        ...
        (expectedType, initialRanges) => {
          ...
          const setRangeStyles = ranges => {
            const headers = INLINE_HEADERS.slice(0, INLINE_HEADERS.indexOf(`inline-${expectedType}`) + 1);
            return ranges.map((range, index) => ({ ...range, style: headers[index % headers.length] }));
          };
