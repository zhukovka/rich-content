...
Adding theme to Ricos is possible with `ricos-theme` packages.

Usage:

```jsx
import { createTheme } from 'ricos-theme';
<RicosEditor theme={createTheme({ palette })} />;
```

ricos-theme exports `createTheme` which gets RicosTheme as argument:
