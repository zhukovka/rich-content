...
The `pluginButtons` parameter is an `inline-button` data array (see divider plugin [inline-buttons](https://github.com/wix-incubator/rich-content/blob/master/packages/plugin-divider/web/src/toolbar/inline-buttons.tsx) for reference).
...
So, the `getToolbarDecorationFn` comes to rescue. This function is expected to return a `Component` to be rendered instead of the default toolbar container. This `Component` will be provided with the toolbar container props. For reference, check out the [InlineToolbarDecoration.jsx](https://github.com/wix-incubator/rich-content/blob/master/examples/main/src/Components/InlineToolbarDecoration.jsx)
